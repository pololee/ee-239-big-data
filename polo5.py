...
def question_3(url, fw_answer_csv):
	...
	for question in question_list:
		...
		for index in range(len(extracted_user_id_list)):
			...
			fw_answer_csv.write(line.encode('utf-8'))
...
def question_5_helper(fw_user_csv, user_url):
	...
	users_line = pure_user_url + comma + number_of_topics + comma + number_of_blogs + comma + number_of_questions + comma + number_of_answers + comma + number_of_edits + comma + followers + comma + following + "\n"
	
	fw_user_csv.write(users_line.encode('utf-8'))
...
def question_6(file_name, line_number):
	count = 0
	chosen_line = ""
	with open(file_name, mode='r') as fr_csv:
		for line in fr_csv:
			count += 1
...
if __name__ == "__main__":
	...
	# test_line_number = 5
	# answers.csv
	# question_6(answer_csv_file, test_line_number)
	# users.csv
	# question_6(user_csv_file, test_line_number)
